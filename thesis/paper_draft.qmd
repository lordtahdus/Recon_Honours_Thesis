...
$$
\hat{\boldsymbol{W}}^{g, sv}_{h} = \boldsymbol{D}^{1/2}_h \hat{\boldsymbol{R}}^g_1 \boldsymbol{D}^{1/2}_h, \quad

\hat{\boldsymbol{R}}^g_1 = D^{-1/2}_1 \, g(\hat{\boldsymbol{W}}_1) \, D^{-1/2}_1,
$$

where $\boldsymbol{D}_h$ is a diagonal matrix comprising $\hat{\sigma}^2_{1,h} , \ldots, \hat{\sigma}^2_{n,h}$ on its diagonal, and $\hat{\sigma}^2_{i,h}$ is $h$-step-ahead base forecast error variance of the $i$-th series. $\hat{\boldsymbol{R}}^g_1$ is the estimated one-step correlation matrix using either shrinkage or NOVELIST estimator.
