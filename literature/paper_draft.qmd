...
#| fig-cap: "Heatmaps of the VAR(1) coefficient matrix and correlation matrix for the 50x2 structure."
#| fig.height: 4
#| fig.width: 6
#| out.width: "85%"
#| fig.align: "center"
...
For each series, $T = 54$ and $304$ observations are generated. The first 50 and 300 observations are used for training, and the last 4 observations are used for testing. The training data is used to compute the best fitted ARIMA models by minimising the AICc criterion, in which we use the automatic algorithm from Fabletools R package [@O-Hara-Wild2024-we]. We refer to them as base models, and their base forecasts are then reconciled using the MinT with different covariance estimators. These include using the unbiased sample covariance matrix - mint_sample, the shrinkage estimator - mint_shr, and the NOVELIST estimator - mint_n. The Monte Carlo simulation is repeated $M = 200$ times, in which the parameters for data generating process is fixed.
...
#| fig-cap: "Relative improvement of the MSE of reconciled forecasts over the base forecasts for the 50x2 hierarchical structure with dense and sparse DGP's correlation matrix, for 1 to 4 steps ahead forecasts, with 2 time series lengths (T = 54 and T = 304)."
#| fig.height: 10
#| fig.width: 10
#| out.width: "90%"
#| fig.align: "center"
...
In this section we explore the effects of "sparse" and "dense" correlation matrix in our data generating process for series at the bottom level. We consider a large structure, having 2 groups of 50 bottom series. The cross-series within-group correlations are strong, and the between-group correlations will be weaker for the "dense" case, and zero for the "sparse" case. The dense and sparse correlation matrices for the DGP of bottom series are illustrated in @fig-Sigma-50x2. Note that these 2 cases have the same VAR(1) structure, only the correlation matrix is different.

However, rather than aggregating series within group to form 2 level-1 series as we did earlier, we combine them in groups of size 10 to obtain 10 level-1 series. These 10 series will be splitted into 3 groups, having 4, 3, and 4 series respectively, and are aggregated into 3 level-2 series. Finally, they are added to obtain the top level. The intention behind this unusual aggregation path is to create a more complex structure for higher levels, in which the level-2 series will correlate with the bottom series in the first or second group, or both.

Results are presented in @fig-sim-results-dsp, with the graphs in the first column being the "dense" case and the ones in second column being "sparse" case. We consider two time series length--54 and 304 observations with the last 4 being the test set--presenting at the first and second row respectively. As shown in the plots, MinT with either Shrinkage (mint_shr) or NOVELIST estimator (mint_n) return significant improvements over the base forecasts, especially in the 1-step-ahead horizon. However, the difference between their accuracy is hardly noticeable, with mint_n slightly outperform at the 1-step-ahead and mint_shr takes over in further forecasting horizons. mint_sample performs worse than base ARIMA models in short time series length, only to improve drastically in longer length. It is due to the sample covariance matrix gets more accurate as sample size gets larger. 
