...
\textcolor{red}{Globally, Starbucks manages thousands of outlets across dozens of countries, each projecting sales for dozens of beverages. Yet when regional, national, and brand-wide forecasts are aggregated, discrepancies of millions of dollars emerge simply because individual outlet forecasts fail to respect the hierarchical nature. Forecast reconciliation comes in to solve this huge problem, where the individual forecasts are adjusted to satisfy the aggregation constraints.} Among the various reconciliation methods, the **MinT** (Minimum Trace) is considered the optimal approach, however, it requires a good estimate of the covariance matrix of the base forecast errors. The current practice is to use the shrinkage estimator (often shrinking toward a diagonal matrix), but it lacks flexibility and might neglect the prominent structure presented. In this project, we aim to assess the forecasting performance of MinT when different covariance estimators are used, namely NOVELIST (NOVEL Integration of the Sample and Thresholded Covariance), POET (Principal Orthogonal complEment Thresholding), and others.
...
The hierarchical structure can be represented as a tree, as shown in @fig-hierarchy. The top level of the tree represents the total value of all series, while the lower levels represent the individual series. When there are attributes of interest that are crossed, such as the electricity demand of any aggregation level (national, state, or regional) can be further broken down by usage purposes (e.g., residential, commercial), it will become a grouped time series (illustrated in @fig-grouped).
...
![Diagram of 2-level grouped structure, and can be considered as the union of two hierarchical trees with common top and bottom level series](D:/Github/Recon_Honours_Thesis/research_proposal/figs/grouped_structure.png){#fig-grouped width="90%" height="40%"}


We refer to both of these structures as hierarchical time series, and can be represented using matrix algebra:
...
\textcolor{red}{If we produce base forecasts for the bottom-level series $\hat{\boldsymbol{b}}_h$ (any method may have been used to produce the base forecasts) and then reconcile them to all aggregated levels $\tilde{\boldsymbol{y}}_h$, we would not be utilising the information from those higher levels. This is similar to the top-down and middle-out approaches, with appropriate summing matrix $\boldsymbol{S}$. Another issue with expressing this way is that it restricts the reconciliation to only single-level approaches. Thus, @Hyndman2011-jv suggested a generalised expression for all existing reconciled methods, which also provides a framework for new methods to be developed:}

$$
\tilde{\boldsymbol{y}}_h = \boldsymbol{S} \boldsymbol{G} \hat{\boldsymbol{y}}_h,
$$ {#eq-2}

for a suitable $n_b \times n$ matrix $\boldsymbol{G}$. $\boldsymbol{G}$ maps the base forecasts of all levels $\hat{\boldsymbol{y}}_h$ down into the bottom series, which is then aggregated to the higher levels by $\boldsymbol{S}$.
...
The proposed the optimal $\boldsymbol{G}$ based on the regression model:
...
An obvious drawback of the OLS solution is that it weights all series in any level equally. The issue prompted @Hyndman2016-ic to propose a WLS solution, where the series are weighted by the inverse variances of the base forecasts. The WLS solution is $\boldsymbol{G} = (\boldsymbol{S}' \boldsymbol{\Lambda}_h^{-1} \boldsymbol{S})^{-1} \boldsymbol{S}' \boldsymbol{\Lambda}_h^{-1}$. $\boldsymbol{\Lambda}_h = \text{diag} (\boldsymbol{W}_h)$ and $\boldsymbol{W}_h = Var(\boldsymbol{y}_h - \hat{\boldsymbol{y}}_h)$.

\color{black}

## Mininum Trace (MinT) Reconciliation

@Wickramasuriya2019-xq reframed the problem by taking an optimisation approach rather than the regression. They formulated the problem as minimising the variances of all reconciled forecasts from @eq-2, which happens to be equivalent to minimising the trace of the covariance matrix (sum of the diagonal elements). This is known as the Minimum Trace (MinT) reconciliation method. The MinT solution is given by
...
@Wickramasuriya2019-xq also showed that MinT is an algebraical generalisation of the GLS, and the OLS and WLS methods are special cases of MinT when $\boldsymbol{W}_h$ is an identity and a diagonal matrix, respectively. However, the MinT solution hinges on a reliable estimate of $\boldsymbol{W}_h$, which is challenging to estimate in high-dimensional setting. Therefore, we will be exploring alternative covariance estimators.
